...
/// to exercise the same coverage against the real CmsisRtos2Thread port --
/// running here on Zephyr's own CMSIS-RTOS v2 compatibility layer
/// (CONFIG_CMSIS_RTOS_V2, subsys/portability/cmsis_rtos_v2/) on native_sim,
/// not the ZephyrThread port itself. Only one line differs from
/// zephyr-linux's copy: SetThreadPriority() takes CMSIS-RTOS2's osPriority_t
/// enum instead of a plain int.
...
static void FullPolicy_Drop_DropsWhenFull()
{
    ...
    // Give the consumer a lower priority than the calling thread. CMSIS-RTOS2's
    // osPriority_t is the opposite convention from Zephyr's raw int priorities
    // (higher number = higher priority; osPriorityNormal, used by the default
    // worker priority and this sample's main thread, is higher than
    // osPriorityLow). With a lower-priority consumer, the scheduler never
    // switches to it just because a message was posted; it only runs once the
    // caller blocks or sleeps, so the "publisher never stalls" assumption
    // below holds.
    dropThread.SetThreadPriority(osPriorityLow);
