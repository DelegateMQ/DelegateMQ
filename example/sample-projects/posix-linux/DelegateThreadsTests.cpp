...
/// unchanged from test/unit-tests/DelegateThreadsTests.cpp to exercise the
/// same coverage against the real POSIX (DMQ_THREAD_POSIX) Thread port
/// instead of the desktop stdlib port. A plain file-scope `static Thread`
/// is safe here (unlike freertos-linux/threadx-linux/zephyr-linux's
/// construct-on-first-use workaround) -- POSIX has no RTOS kernel-start
/// ordering hazard; the underlying pthread primitives work at static-init
/// time same as any other point in the process's life.
