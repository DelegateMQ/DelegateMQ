...
/// zephyr-linux (itself ported unchanged from test/unit-tests/DelegateThreadsTests.cpp)
/// to exercise the same coverage against the real NuttX Thread port
/// (pthread_create/pthread_join, POSIX mqueue) on NuttX's `sim` board.
...
// Construct-on-first-use: matches the same pattern zephyr-linux/cmsis-rtos2-linux
// use, deferring construction until the first call from DelegateThreadsTests(),
// well after NuttX's own startup is complete.
...
// dmq::Mutex resolves to a real pthread_mutex_t on NuttX -- NuttX's `sim`
// board schedules genuinely preemptively (unlike Zephyr's native_sim
// cooperative single-token model, which needed dmq::Mutex specifically to
// avoid a real deadlock class), so an ordinary mutex works exactly as it
// would on any other preemptive RTOS/pthread platform here.
...
{
    // std::random_device needs /dev/urandom, which this minimal NuttX sim
    // config doesn't provide (no device-file backend mounted). This value is
    // just jitter for a stress test, not a security-sensitive seed, and this
    // function is called concurrently from multiple worker threads -- so,
    // matching the original per-call-local (never shared/static) design,
    // seed a fresh generator each call from the steady clock instead, which
    // needs no OS entropy source and naturally differs call to call.
...
static void FullPolicy_Drop_DropsWhenFull()
{
    ...
    // Give the consumer a lower priority than the calling thread. NuttX uses
    // the standard POSIX SCHED_FIFO convention (higher number = higher
    // priority): this sample's entry task and NuttXThread's own default
    // worker priority are both 100, so this must be set explicitly below
    // that to actually be lower. With a lower-priority consumer, NuttX's
    // preemptive scheduler never switches to it just because a message was
    // posted; it only runs once the caller blocks or sleeps, so the
    // "publisher never stalls" assumption below holds.
    dropThread.SetThreadPriority(50);
