...
NetworkMgr::NetworkMgr()
    : m_retryMonitor(m_sendTransport, m_dispatcher.GetTransportMonitor())
...
{
    // Hand our transports to m_dispatcher. Raw UDP has no reliability of
    ...
    m_dispatcher.Attach(m_reliableTransport, m_recvTransport);
    ...
    if (!m_dispatcher.GetThread().IsCurrentThread())
        return dmq::MakeDelegate(this, &NetworkMgr::OpenTransport, m_dispatcher.GetThread(), dmq::WAIT_INFINITE)();
...
void NetworkMgr::SendAlarmMsg(AlarmMsg& msg, AlarmNote& note) {
    if (!m_dispatcher.GetThread().IsCurrentThread()) {
        dmq::MakeDelegate(this, &NetworkMgr::SendAlarmMsg, m_dispatcher.GetThread())(msg, note);
        return;
    }
...
void NetworkMgr::SendCommandMsg(CommandMsg& command) {
    if (!m_dispatcher.GetThread().IsCurrentThread()) {
        dmq::MakeDelegate(this, &NetworkMgr::SendCommandMsg, m_dispatcher.GetThread())(command);
        return;
    }
...
void NetworkMgr::SendDataMsg(DataMsg& data) {
    if (!m_dispatcher.GetThread().IsCurrentThread()) {
        dmq::MakeDelegate(this, &NetworkMgr::SendDataMsg, m_dispatcher.GetThread())(data);
        return;
    }
    (*m_dataChannel)(data);
}
...
void NetworkMgr::SendActuatorMsg(ActuatorMsg& msg) {
    if (!m_dispatcher.GetThread().IsCurrentThread()) {
        dmq::MakeDelegate(this, &NetworkMgr::SendActuatorMsg, m_dispatcher.GetThread())(msg);
        return;
    }
