/**
 * @file safety/main_threadx.cpp
 * @brief Safety CPU — Independent Hardware Monitor & Interlock (ThreadX build)
 *
 * ThreadX equivalent of main.cpp. Everything below this file -- System.h and
 * the safety monitoring logic -- is untouched and identical to the FreeRTOS
 * build; only the kernel init/task-creation glue here differs. See main.cpp
 * for the FreeRTOS version of this same glue.
 */
...
#define SAFETY_STACK_SIZE   65536
#define WATCHDOG_STACK_SIZE 16384

static TX_TIMER g_sysTimer;
static TX_THREAD g_safetyThread;
static TX_THREAD g_watchdogThread;
static UCHAR g_safetyStack[SAFETY_STACK_SIZE];
...
static void SafetyThreadEntry(ULONG) {
...
extern "C" void tx_application_define(void* /*first_unused_memory*/) {
    ...
    printf("Cellutron Safety Processor starting (ThreadX Simulator)...\n");

    // Priority 0 = highest urgency on ThreadX; matches the FreeRTOS build's
    // top-priority Safety/Watchdog tasks. See Constants.h for the app
    // subsystem threads' (System/Network) priority mapping.
    tx_thread_create(&g_safetyThread, const_cast<CHAR*>("Safety"), SafetyThreadEntry, 0,
                      g_safetyStack, SAFETY_STACK_SIZE, 0, 0, TX_NO_TIME_SLICE, TX_AUTO_START);
