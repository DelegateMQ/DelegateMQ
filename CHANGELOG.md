...
### Added
- New `example/sample-projects/zephyr-udp-serializer/` sample: a remote delegate call over `ZephyrUdpTransport` (Zephyr's native BSD-socket networking), both `Sender` and `Receiver` in one `native_sim` process communicating over real UDP loopback (127.0.0.1) sockets — the first time this transport has actually been built and run. `sender.h`/`receiver.h`/`data.h`/`data.cpp` are copied unchanged from `linux-udp-serializer`, made possible by adding `using UdpTransport = ZephyrUdpTransport;` (matching the alias `LinuxUdpTransport.h`/`Win32UdpTransport.h` already define) so application code stays portable across `DMQ_TRANSPORT_*` the same way `dmq::os::Thread` already is across `DMQ_THREAD_*`. Manual-only, same as `zephyr-linux` — excluded from `03_generate_samples.py`/`04_build_samples.py`.

### Fixed
- **Every RTOS simulator sample (`freertos-linux`/`threadx-linux`/`zephyr-linux`) silently compiled in desktop-only `DataBus`/`NetworkConnect.h` code** — both `Defaults.cmake`'s CMake-level default and `DelegateOpt.h`'s internal preprocessor fallback defaulted `DMQ_DATABUS` on whenever the *host* platform was Windows/Linux/macOS/Unix, which is true for every one of these samples since they all compile with a host toolchain despite targeting an embedded `DMQ_THREAD_*`. Harmless on its own (unused dead code), but `NetworkConnect.h` unconditionally includes the host's own BSD socket headers, which collide outright with a target's native network stack headers (e.g. Zephyr's `<zephyr/net/socket.h>` redefining `sockaddr_in` et al.) the moment a sample also needs real networking — exactly what broke bringing up `zephyr-udp-serializer`. Both defaults now key off the selected `DMQ_THREAD_*` instead of the host platform; RTOS/bare-metal targets that genuinely want DataBus (e.g. `databus-freertos/server`) already opt in explicitly.
- **`ZephyrUdpTransport.h` mixed unprefixed BSD socket names (`socket()`, `bind()`, `sendto()`, ...) with already-`zsock_`-prefixed calls in the same file** — the unprefixed names need `CONFIG_POSIX_API=y` (their predecessor, `CONFIG_NET_SOCKETS_POSIX_NAMES`, was deprecated in Zephyr 3.7 and removed), which replaces large parts of the standard header set — pthread types included — with Zephyr's own POSIX compatibility layer. That layer collides with the *host's* real pthread/libstdc++ headers on `native_sim`, which links against them directly for C++ support. Rewritten to use `zsock_*` throughout, which needs no such layer and is Zephyr's current recommended API regardless of target.
- **`ZephyrUdpTransport::Create()` failed unconditionally** — `setsockopt(SO_RCVTIMEO)` returned `ENOPROTOOPT` because `CONFIG_NET_CONTEXT_RCVTIMEO` (off by default) was never enabled; `Create()`'s return value goes unchecked in `sender.h`/`receiver.h` (matching every other `*-udp-serializer` sample), so both sockets silently closed and every send failed with `ERR_DISPATCH`. Root-caused with targeted debug prints tracing the failure from `Dispatcher::Dispatch()` through `RetryMonitor::SendWithRetry()` into `Create()` itself; fixed via `example/sample-projects/zephyr-udp-serializer/prj.conf`.
- **`DelegateMQ.h`'s `DMQ_TRANSPORT_THREADX_UDP` `#include` pointed at a nonexistent directory** — `"port/transport/threadx-udp/NetXUdpTransport.h"`, but the file has always lived at `port/transport/netx-udp/NetXUdpTransport.h`; selecting this transport would fail to compile immediately. Never caught because nothing in this repo has ever built it — same `port/transport/README.md` entry corrected to match.

### Added
- New `example/sample-projects/databus-zephyr/` sample: the Zephyr counterpart to `databus-freertos` — a `native_sim` server and a standard C++ client, in **genuinely separate processes**, exchanging real `dmq::databus::DataBus` traffic (sensor readings, alarm state, rate-control commands) over UDP. `Server.h`/`main_delegate.cpp` are a fresh Zephyr port of `databus-freertos/server/Server.h`; `common/` (`Msg.h`/`MsgSerializer.h`) and `client/` (`Client.h`/`main.cpp`) are copied over with only naming/wording changes. Verified bidirectional: the client receives `sensor/temp`/`alarm/status`, and the server visibly changes its publish interval on receiving the client's `cmd/rate` command, across repeated runs. Manual-only (server needs `west build`) — excluded from `03_generate_samples.py`/`04_build_samples.py` as one unit, matching how `databus-freertos` is already excluded on non-Windows hosts.

### Fixed
- **`databus-freertos/client/Client.h` hardcoded `dmq::transport::Win32UdpTransport`** despite its own doc comment claiming "Runs on Linux or Windows" — the client's `CMakeLists.txt` never sets `DMQ_TRANSPORT` explicitly, so it auto-detects `DMQ_TRANSPORT_LINUX_UDP` on Linux, where `Win32UdpTransport` doesn't even exist; the client failed to compile on Linux. Now uses the generic `dmq::transport::UdpTransport` alias (already defined by every desktop transport header) so it resolves correctly per-platform — confirmed building and running on Linux. Reused unchanged for `databus-zephyr/client`.
- **`extras/util/NetworkConnect.h` (pulled in unconditionally by `DataBus.h`) had the same host-platform-vs-target-thread confusion already fixed for `DMQ_DATABUS`'s default** — it included the host's own BSD/Winsock socket headers whenever `_WIN32`/`__linux__`/`__unix__`/`__APPLE__` was defined, true for `native_sim`'s host-GCC build despite targeting Zephyr, colliding with Zephyr's own `<zephyr/net/socket.h>` the moment `DataBus` (needed for `databus-zephyr`) and `ZephyrUdpTransport` (or any embedded UDP transport) were both in play. Now excluded whenever an embedded `DMQ_THREAD_*` is active, same as the `DMQ_DATABUS` default; `NetworkContext::GetLocalAddress()` (the only thing that needed those headers) simply returns `"127.0.0.1"` there, since it's a desktop-only convenience with no embedded equivalent and no caller on these targets.
- **`databus-interop`'s Python and C# clients never updated their `DmqDataBus.start()`/`.Start()` call after it gained a required `multicastGroup` parameter** — `python-client/main.py` crashed immediately (`TypeError: missing 1 required positional argument`); `csharp-client/Program.cs` had the identical bug, which would fail to compile outright. Both now pass `"239.1.1.1"` (matching the group address convention already used by the `databus-multicast` sample) — the C++ server in `../server/` only ever sends plain unicast, and `MulticastTransport`'s SUB side binds `INADDR_ANY` before joining the group, so it receives that unicast traffic regardless; the group itself is otherwise unused here. Verified end-to-end on the Python side (repeated runs: server and client exchange `DataMsg`/`CommandMsg` correctly, including the server's publish rate visibly changing on command); the C# fix is correct by signature inspection but unverified by build — this machine only has the .NET 6 SDK installed, and the project targets `net10.0` (pre-existing, unrelated to this bug).
