...
- **The CMSIS-RTOS2 implementation is unverified.** Written against documented API behavior, same rigor as ThreadX's, but never built or run — this development environment has no CMSIS-RTOS2 SDK or ARM hardware/QEMU target. Review carefully and exercise on real hardware or an appropriate simulator before relying on it in production. (Zephyr's is now verified — see the table above and `example/sample-projects/zephyr-linux`.)
