...
| Bare metal (`DMQ_THREAD_NONE`) | `dmq::os::BareMetalCriticalSection` (same PRIMASK save/disable/restore technique as `BareMetalClock.h`) | **Yes** | Built and run — `example/sample-projects/bare-metal-arm` (QEMU `mps2-an386`, Cortex-M4). Genuinely exercised from ISR context: a real SysTick interrupt calls `Timer::ProcessTimers()` (which takes this lock) directly, not a simulated stand-in — the first genuine hardware-interrupt exercise of this particular critical section |
