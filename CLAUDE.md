...
| NuttX | `dmq::os::NuttXCriticalSection` (`up_irq_save()`/`up_irq_restore(flags)`, NuttX's own architecture-portable interrupt-masking primitive, not a `pthread_mutex_t`) | Yes, per documented NuttX architecture-layer API — but only reachable from application code in a **FLAT** build; a PROTECTED/KERNEL build has no access from userspace | **Unverified** — no NuttX toolchain or simulator (`sim` target) available in this development environment; reasoned through only |
...
- **The entire NuttX port (`port/os/nuttx/`) is unverified** — no NuttX toolchain or simulator (`nuttx/boards/sim`) is available in this development environment to build or run any of it, not just `NuttXCriticalSection`. It is written against documented NuttX POSIX API behavior (the same rigor as the CMSIS-RTOS2/Zephyr ports before they were exercised for real), reusing NuttX's genuine POSIX primitives directly (`pthread_mutex_t`, `sem_t`, POSIX `mqueue`) rather than inventing bespoke wrappers. Review carefully and exercise on real NuttX hardware or `sim` before relying on it in production; a `nuttx-sim`-style companion sample (mirroring `zephyr-linux`/`cmsis-rtos2-linux`) would be the natural way to verify it.
