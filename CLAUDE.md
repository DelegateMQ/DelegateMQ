...
| NuttX | `dmq::os::NuttXCriticalSection` (`up_irq_save()`/`up_irq_restore(flags)`, NuttX's own architecture-portable interrupt-masking primitive, not a `pthread_mutex_t`) | **Yes, confirmed reachable from application code in a FLAT build** (a PROTECTED/KERNEL build still has no userspace access — not applicable to `sim`, which is FLAT) | Built and run — `example/sample-projects/nuttx-sim` (NuttX's own `sim` board, a native host process, `CONFIG_LIBCXX`). `up_irq_save()`/`up_irq_restore()` genuinely work from this port's application-level `Timer::GetLock()` call site: Test 8's one-shot `Timer` fires exactly once, and Test 9/10's cross-thread `FullPolicy`/`PacedDispatch` stress tests all pass. Not exercised from a genuine hardware ISR (no real interrupt controller under `sim`) — only from ordinary thread context, same caveat as CMSIS-RTOS2/Zephyr's own `native_sim`-based verification |
| Bare metal (`DMQ_THREAD_NONE`) | `dmq::os::BareMetalCriticalSection` — architecture-branched: ARM (PRIMASK save/disable/restore, same technique `BareMetalClock.h` uses) or RISC-V (`mstatus.MIE` save/clear/restore via `csrrci`/`csrs`) | **Yes, both architectures** | Built and run — ARM: `example/sample-projects/bare-metal-arm` (QEMU `mps2-an386`, Cortex-M4). RISC-V: `example/sample-projects/bare-metal-riscv` (QEMU `virt`, RV32IMC). Both genuinely exercised from ISR context: a real hardware timer interrupt (SysTick on ARM, CLINT `mtime`/`mtimecmp` on RISC-V) calls `Timer::ProcessTimers()` (which takes this lock) directly, not a simulated stand-in |
...
- **Porting bare metal (`DMQ_THREAD_NONE`) to a new architecture**: `BareMetalCriticalSection.h`/`BareMetalClock.h` branch internally per architecture (`#if defined(__riscv)` / `#elif defined(__arm__) && ...` / ARMCC / fallback) rather than each architecture getting its own file — follow that pattern for a new one (e.g. Xtensa, AVR) rather than creating a parallel `port/os/bare-metal-<arch>/` tree. Check the architecture macro *before* any generic `__GNUC__`/`__clang__` check, since those are true for every GCC/Clang target regardless of architecture — checking compiler before architecture is what caused RISC-V to briefly try to assemble ARM `cpsid`/`cpsie` opcodes when this port was extended for RISC-V.
...
- **The NuttX port (`port/os/nuttx/`) is now verified** — built and run via `example/sample-projects/nuttx-sim` (NuttX's own `sim` board, native host process, no cross-compiler/hardware). `NuttXThread`/`Mutex`/`Semaphore`/`DelegateQueue`/`Clock`/`CriticalSection` all genuinely exercised: cross-thread async dispatch, `MulticastDelegateSafe`, a real one-shot `Timer` (via `NuttXCriticalSection`), and the full `FullPolicy`/`PacedDispatch` stress-test suite (Tests 1-10) all pass. See the sample's README for what this took to get building (a working `libstdc++`/RTTI toolchain for NuttX's own build, and two real NuttX/mkdeps build-system quirks worked around, not DelegateMQ bugs).
