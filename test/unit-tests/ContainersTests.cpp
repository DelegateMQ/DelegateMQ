...
static void UnicastDelegateTests()
{
    // Verify Constructor(DelegateType&&)
    {
        ...
        DMQ_ASSERT_TRUE(directInit.Size() == 1);
        DMQ_ASSERT_TRUE(!directInit.Empty());
    ...
    DMQ_ASSERT_TRUE(src.Size() == 1);
    auto dest = std::move(src);
    DMQ_ASSERT_TRUE(src.Size() == 0);
    DMQ_ASSERT_TRUE(dest.Size() == 1);

    UnicastDelegate<void(int)> src2(dest);
    DMQ_ASSERT_TRUE(src2.Size() == 1);
    DMQ_ASSERT_TRUE(dest.Size() == 1);
...
static void MulticastDelegateTests()
{
    // Verify Constructor(DelegateType&&)
    {
        ...
        DMQ_ASSERT_TRUE(directInit.Size() == 2);
    }
    ...
    src -= MakeDelegate(lambda1);
    src(TEST_INT);
    DMQ_ASSERT_TRUE(funcInt == TEST_INT);
    DMQ_ASSERT_TRUE(classInt == TEST_INT);
    DMQ_ASSERT_TRUE(lambda1Int == 0);
    DMQ_ASSERT_TRUE(lambda2Int == TEST_INT);
    DMQ_ASSERT_TRUE(src.Size() == 3);
    ...
    DMQ_ASSERT_TRUE(funcInt == TEST_INT);
    DMQ_ASSERT_TRUE(classInt == TEST_INT);
    DMQ_ASSERT_TRUE(lambda1Int == TEST_INT);
    DMQ_ASSERT_TRUE(lambda2Int == 0);
    DMQ_ASSERT_TRUE(src.Size() == 3);
...
static void MulticastReentrantTests()
{
    // Test 1: Self-Removal (A delegate removes itself during invocation)
    {
        ...
        DMQ_ASSERT_TRUE(del.Size() == 1);

        // Invoke: Should run, increment count, then safely remove itself without crashing
        del(TEST_INT);

        // Verification
        DMQ_ASSERT_TRUE(g_ReentrantRunCount == 1); // It ran once
        DMQ_ASSERT_TRUE(del.Size() == 0);          // It was successfully removed after execution
        DMQ_ASSERT_TRUE(del.Empty());
    }
    ...
    // Test 2: Remove Next (Delegate A removes Delegate B before B runs)
    {
        ...
        // Verification
        DMQ_ASSERT_TRUE(g_ReentrantRunCount == 1); // Only Tester1 should have run
        DMQ_ASSERT_TRUE(del.Size() == 1);          // Tester2 should be gone, Tester1 remains
    }
    ...
    // Test 3: Recursive Broadcast (A delegate triggers another broadcast)
    {
        ...
        DMQ_ASSERT_TRUE(recursiveCount == 3);
...
static void MulticastDelegateSafeReentrantTests()
{
    ...
    // Test 1: Self-Removal Safe
    {
        ...
        // Verification
        DMQ_ASSERT_TRUE(g_ReentrantRunCount == 1);
        DMQ_ASSERT_TRUE(del.Size() == 0);
    }
}
...
static void SignalTests()
{
    // Test 1: Manual Disconnect
    {
        ...
        DMQ_ASSERT_TRUE(conn.IsConnected());
    ...
    // Test 2: RAII Scoped Disconnect
    {
        ...
        int callCount = 0;
        {
            ...
            DMQ_ASSERT_TRUE(scopedConn.IsConnected());
            sig(&callCount);
            DMQ_ASSERT_TRUE(callCount == 1);

        } // Destructor runs

        DMQ_ASSERT_TRUE(sig.Size() == 0);
        sig(&callCount);
        DMQ_ASSERT_TRUE(callCount == 1);
    }
    ...
    // Test 3: Move Semantics (move-construction)
    {
        ...
        DMQ_ASSERT_TRUE(conn1.IsConnected());

        // Explicit move
        ScopedConnection conn2 = std::move(conn1);

        DMQ_ASSERT_TRUE(conn2.IsConnected());
        DMQ_ASSERT_TRUE(!conn1.IsConnected()); // conn1 is now empty
        DMQ_ASSERT_TRUE(sig.Size() == 1);
    ...
    // Test 5: Multiple subscribers — all fire, individual disconnect works
    {
        ...
        DMQ_ASSERT_TRUE(sig.Size() == 3);
        DMQ_ASSERT_TRUE(!sig.Empty());

        sig(&cnt1);
        DMQ_ASSERT_TRUE(cnt1 == 3); // all three incremented the same counter

        // Disconnect middle subscriber; remaining two still fire
        c2.Disconnect();
        DMQ_ASSERT_TRUE(sig.Size() == 2);

        cnt1 = 0;
        sig(&cnt1);
        DMQ_ASSERT_TRUE(cnt1 == 2);
    }
    ...
    // Test 6: Empty() and Clear()
    {
        ...
        DMQ_ASSERT_TRUE(sig.Empty());
        DMQ_ASSERT_TRUE(sig.Size() == 0);
        DMQ_ASSERT_TRUE(c1.IsConnected());
        DMQ_ASSERT_TRUE(c2.IsConnected());
