...
{
    MonotonicGuard<uint64_t> g;
    DMQ_ASSERT_TRUE(g.IsNewer(100));
    DMQ_ASSERT_TRUE(g.IsNewer(101));
    DMQ_ASSERT_TRUE(g.IsNewer(999));
    DMQ_ASSERT_TRUE(g.IsNewer(std::numeric_limits<uint64_t>::max() - 1));
...
{
    MonotonicGuard<uint64_t> g;
    DMQ_ASSERT_TRUE(g.IsNewer(50));
    DMQ_ASSERT_TRUE(!g.IsNewer(50));  // same timestamp
...
{
    MonotonicGuard<uint64_t> g;
    DMQ_ASSERT_TRUE(g.IsNewer(500));
    DMQ_ASSERT_TRUE(!g.IsNewer(499));
    DMQ_ASSERT_TRUE(!g.IsNewer(0));
...
{
    MonotonicGuard<uint64_t> g;
    DMQ_ASSERT_TRUE(g.IsNewer(200));
    DMQ_ASSERT_TRUE(!g.IsNewer(100));   // stale before reset
    g.Reset();
    DMQ_ASSERT_TRUE(g.IsNewer(100));    // first call after reset always succeeds
    DMQ_ASSERT_TRUE(g.IsNewer(101));
    DMQ_ASSERT_TRUE(!g.IsNewer(100));   // back to normal monotonic behaviour
...
static void MonotonicGuard_U64_FirstCallWithMaxValue()
{
    ...
    MonotonicGuard<uint64_t> g;
    DMQ_ASSERT_TRUE(g.IsNewer(std::numeric_limits<uint64_t>::max()));
    DMQ_ASSERT_TRUE(!g.IsNewer(std::numeric_limits<uint64_t>::max()));  // duplicate
...
static void MonotonicGuard_U32_NormalIncreasing()
{
    ...
    DMQ_ASSERT_TRUE(g.IsNewer(1000));
    DMQ_ASSERT_TRUE(g.IsNewer(1001));
    DMQ_ASSERT_TRUE(!g.IsNewer(1001));  // duplicate
    DMQ_ASSERT_TRUE(!g.IsNewer(500));   // stale
...
static void MonotonicGuard_U32_StaleAfterRollover()
{
    ...
    const uint32_t nearMax = std::numeric_limits<uint32_t>::max() - 10;
    DMQ_ASSERT_TRUE(g.IsNewer(nearMax));
    DMQ_ASSERT_TRUE(g.IsNewer(5));           // rollover accepted
    DMQ_ASSERT_TRUE(!g.IsNewer(4));          // duplicate/stale after rollover
    DMQ_ASSERT_TRUE(!g.IsNewer(3));
...
static void MonotonicGuard_U8_RolloverDetected()
{
    ...
    DMQ_ASSERT_TRUE(g.IsNewer(250));
    DMQ_ASSERT_TRUE(g.IsNewer(5));   // rolled over; signed diff = 5 - 250 as int8 = positive
    DMQ_ASSERT_TRUE(!g.IsNewer(4));  // stale
...
static void MonotonicGuard_U8_NoFalseRollover()
{
    ...
    MonotonicGuard<uint8_t> g;
    DMQ_ASSERT_TRUE(g.IsNewer(10));
    // delta = 128; static_cast<int8_t>(128) = -128 → rejected
    DMQ_ASSERT_TRUE(!g.IsNewer(static_cast<uint8_t>(10 + 128)));  // = 138
